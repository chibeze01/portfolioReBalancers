...
from ...pricing.yahoo_provider import get_stock_info_batch
...
def _analyze_portfolio_sectors(holdings: list) -> Dict[str, Decimal]:
    ...
    try:
        batch_info = get_stock_info_batch(symbols)
    except Exception as e:
        logger.warning(f"Could not get batch sector info: {e}")
        batch_info = {}

    for holding in holdings:
        try:
            info = batch_info.get(holding.symbol.upper())
            ...
            logger.warning(f"Could not calculate sector info for {holding.symbol}: {e}")
