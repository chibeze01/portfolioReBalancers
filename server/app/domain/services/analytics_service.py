...
def portfolio_pnl(db: Session, user_id: str, portfolio_id: uuid.UUID) -> PnLResponse:
    ...
    total = Decimal("0")
    for h in p.holdings:
        price = batched_prices.get(h.symbol.upper()) or get_price(h.symbol)
...
def portfolio_historical(db: Session, user_id: str, portfolio_id: uuid.UUID, days: int = 30) -> HistoricalPortfolioResponse:
    ...
    for h in p.holdings:
        history = batched_historical.get(h.symbol.upper())  # extra days buffer
        current_price = batched_prices.get(h.symbol.upper()) or get_price(h.symbol)
