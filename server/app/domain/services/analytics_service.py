...
def portfolio_historical(db: Session, user_id: str, portfolio_id: uuid.UUID, days: int = 30) -> HistoricalPortfolioResponse:
    ...
    # Fetch historical prices for each holding in batch
    batched_history = get_historical_prices_batch(symbols, days + 5)

    holding_prices = {}
    current_value = Decimal("0")

    for h in p.holdings:
        history = batched_history.get(h.symbol.upper())  # extra days buffer
