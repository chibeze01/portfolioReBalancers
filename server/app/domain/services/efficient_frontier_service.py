...
def _get_prices_batch_local(symbols: list[str]) -> dict[str, Decimal]:
...
def _load_portfolio_data(db: Session, user_id: str, portfolio_id: uuid.UUID):
    ...
    returns_matrix = []
    historical_data = get_historical_prices_batch(symbols, 365)
    for symbol in symbols:
        history = historical_data.get(symbol.upper())
    ...
    current_prices = _get_prices_batch_local(symbols)
    for symbol in symbols:
